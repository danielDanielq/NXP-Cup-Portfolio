...
#define DISTANCE_SENSOR1_TRIG_PIN 5
#define DISTANCE_SENSOR1_ECHO_PIN 4
...
void trigger_pulse()
{
      ...
      switch(state)                                  // State machine handles delivery of trigger pulse
      {
        ...
        case 1:                                      // Initiate pulse
           digitalWrite(DISTANCE_SENSOR1_TRIG_PIN, HIGH);              // Set the trigger output high
           state = 2;                                // and set state to 2
           break;
        
        case 2:                                      // Complete the pulse
        default:      
           digitalWrite(DISTANCE_SENSOR1_TRIG_PIN, LOW);               // Set the trigger output low
...
void echo_interrupt()
{
  switch (digitalRead(DISTANCE_SENSOR1_ECHO_PIN))                     // Test to see if the signal is high or low
  {
...
void setup() 
{
  pinMode(DISTANCE_SENSOR1_TRIG_PIN, OUTPUT);                           // Trigger pin set to output
  pinMode(DISTANCE_SENSOR1_ECHO_PIN, INPUT);                            // Echo pin set to input
  pinMode(onBoardLED, OUTPUT);                        // Onboard LED pin set to output
  
  Timer1.initialize(TIMER_US);                        // Initialise timer 1
  Timer1.attachInterrupt( timerIsr );                 // Attach interrupt to the timer service routine 
  attachInterrupt(digitalPinToInterrupt(DISTANCE_SENSOR1_ECHO_PIN), echo_interrupt, CHANGE);  // Attach interrupt to the sensor echo input
