...
// polynomial_degree = 2
[[maybe_unused]]static Point2D polyval(float* polynomial_coefficients, int polynomial_degree, float x) {
	float y = 0.0f;
	float x_power = 1.0f;
	Point2D result;
...
[[maybe_unused]]static int gaussianElimination2(float A[2][2 + 1], float x[2], int n) {
	int j, i, k;
	int pivot_row;
	float factor;
	float temp;
...
[[maybe_unused]]static ParabolaABC points2parabola_3(Point2D point1, Point2D point2, Point2D point3) {
...
[[maybe_unused]]static int isValidParabola(ParabolaABC parabola) {
...
[[maybe_unused]]static Point2D parabolaVertex(ParabolaABC parabola) {
	Point2D vertex;
	float delta;
	// (h, k) = (-b/2a, -D/4a)
...
// Function to check if a point (px, py) is inside the triangle
int is_point_in_triangle(float x1, float y1, float x2, float y2, float x3, float y3, float px, float py) {
	float A = triangle_area(x1, y1, x2, y2, x3, y3); // Area of full triangle
	float A1 = triangle_area(px, py, x2, y2, x3, y3); // Area of sub-triangle (P, B, C)
	float A2 = triangle_area(x1, y1, px, py, x3, y3); // Area of sub-triangle (A, P, C)
	float A3 = triangle_area(x1, y1, x2, y2, px, py); // Area of sub-triangle (A, B, P)
	
	// Check if the sum of sub-triangle areas equals the main triangle area
	return fabsf(A - (A1 + A2 + A3)) < 0.00001f;
}

int is_point_in_rectangle(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float px, float py) {
	// A rectangle is divided into two triangles: (A, B, C) and (A, C, D)
	return is_point_in_triangle(x1, y1, x2, y2, x3, y3, px, py) || is_point_in_triangle(x1, y1, x3, y3, x4, y4, px, py);
}
...
[[maybe_unused]]static int arePerpenticularABC(LineABC line1, LineABC line2) {
	line2 = normalizeLineABC2MQ(line2);
	line1 = normalizeLineABC2MQ(line1);
...
static float minDistanceLineSegmentToLine(LineSegment segment, LineABC line) {
    float distanceA = distance2lineABC(segment.A, line);
    float distanceB = distance2lineABC(segment.B, line);

    // Handle potential NaN or infinite values
    if (std::isnan(distanceA) || std::isinf(distanceA)) distanceA = FLT_MAX;
    if (std::isnan(distanceB) || std::isinf(distanceB)) distanceB = FLT_MAX;

    return std::min(distanceA, distanceB);
}

[[maybe_unused]]static int isPointOnLineABC(Point2D point, LineABC lineAbc) {
	float result;
...
[[maybe_unused]]static float distanceBwLinesABC(LineABC line1, LineABC line2, Point2D pointOnLine) {
...
[[maybe_unused]]static LineSegmentsDistancePoints distancePointsBwSegments(LineSegment segment1, LineSegment segment2) {
	...
	[[maybe_unused]]int projectionPresentSeg1A, projectionPresentSeg1B, projectionPresentSeg2A, projectionPresentSeg2B, minFound, maxFound, minSet, maxSet;
