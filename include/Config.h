...
static int enable_finish_line_detection_soft = 0;
...
static float min_speed = 117.0f;
static float max_speed = 150.0f;
static float emergency_break_distance_cm = 75.0f;
static float emergency_brake_min_speed = 100.0f;
...
static float car_speed_ki = -0.03f;
static float car_speed_kd = -0.4f;
static float car_speed_ki_min_max_impact = 5.0f;
static float max_error_angle_degrees = 15.0f;
static float max_error_distance_finish_line = 10.0f;
static float max_speed_after_finish_line = 115.0f;

#if RACE_MODE == 1
  static float emergency_brake_enable_delay_s = 8.0f;
#elif DEBUG_MODE == 1
...
#define SERIAL_PORT_CONFIGURATION 2

#if SERIAL_PORT_CONFIGURATION == 1
  #define SERIAL_PORT_TYPE usb_serial_class
  #define SERIAL_PORT Serial
#elif SERIAL_PORT_CONFIGURATION == 2
...
#define ENABLE_WAIT_FOR_SERIAL_PORT 1
...
#define ESCAPED_CHARACTER_AT_BEGINNING_OF_STRING '%'
  ...
  #define ENABLE_REMOTE_START_STOP 0
  #define ENABLE_DETATCH_MENU_AFTER_START_CAR_ENGINE 0
  #define ENABLE_WAIT_FOR_SERIAL_PORT 0
...
#define MAX_ERROR_ANGLE_DEGREES max_error_angle_degrees
#define MAX_ERROR_DISTANCE_FINISH_LINE max_error_distance_finish_line
#define MAX_SPEED_AFTER_FINISH_LINE max_speed_after_finish_line
...
#define LANE_WIDTH_VECTOR_UNIT (float)(LANE_WIDTH_VECTOR_UNIT_REAL)
  ...
  #define STEERING_SERVO_ANGLE_MIDDLE     90    // 90 middle // 120(90)
  #define STEERING_SERVO_ANGLE_MAX_RIGHT  35    // 0 max right // 90 - 58 = 32(35)
  #define STEERING_SERVO_ANGLE_MAX_LEFT   145   // 180 max left // 90 + 58(145)
...
static void printDataToSerial(Vector leftVectorOld, Vector rightVectorOld, Vector leftVector, Vector rightVector, LineABC leftLine, LineABC rightLine, LineABC laneMiddleLine, PurePursuitInfo purePersuitInfo, float carAcceleration, float frontObstacleDistance, float carSpeed_, float loop_time_ms_, bool finishLineDetected_, FinishLine finishLine_){
  String commaCharStr;
  char semicolonChar;
  ...
  SERIAL_PORT.print(semicolonChar);
  SERIAL_PORT.print(String(loop_time_ms_));
  SERIAL_PORT.print(semicolonChar);
  SERIAL_PORT.print(String(finishLineDetected_));
  SERIAL_PORT.print(semicolonChar);
  SERIAL_PORT.print(String(finishLine_.leftSegment.m_x0) + commaCharStr + String(finishLine_.leftSegment.m_y0) + commaCharStr + String(finishLine_.leftSegment.m_x1) + commaCharStr + String(finishLine_.leftSegment.m_y1));
  SERIAL_PORT.print(semicolonChar);
  SERIAL_PORT.print(String(finishLine_.rightSegment.m_x0) + commaCharStr + String(finishLine_.rightSegment.m_y0) + commaCharStr + String(finishLine_.rightSegment.m_x1) + commaCharStr + String(finishLine_.rightSegment.m_y1));
...
[[maybe_unused]]static void HardwareReset(){
  delay(100);
  SCB_AIRCR = 0x05FA0004;
}
/*==============================================================================*/

bool readRecordFromSerial(SERIAL_PORT_TYPE& serialPort, String recordTermintor, std::vector<char> &record){
...
void parseAndSetGlobalVariables(std::vector<char>& rawData, char variableTerminator = ';') {
  ...
  max_error_angle_degrees = parseNextFloat(pEnd, (rawData.size() + rawData.data()) - pEnd, variableTerminator, &pEnd, &resultSuccess);
...
void printGlobalVariables(SERIAL_PORT_TYPE& serialPort){
  ...
  serialPort.print(separatorCharacter);
  serialPort.print(String(max_error_angle_degrees));
  serialPort.print(separatorCharacter);
  serialPort.print(String(max_error_distance_finish_line));
...
void settingsMenuRoutine(LiquidCrystal_I2C &lcd_, int left_arrow_btn, int right_arrow_btn, int increment_btn, int decrement_btn) {
  enum LcdMenu {LCDMENU_FIRST_VALUE,
                ...
                LCDMENU_MAX_ERROR_ANGLE_DEGREES,
                LCDMENU_MAX_ERROR_DISTANCE_FINISH_LINE,
  ...
  [[maybe_unused]]static float lcd_print_timeont = 0.0f;
  ...
  if (leftArrowButtonState == HIGH || rightArrowButtonState == HIGH || incrementButton == HIGH || decrementButton == HIGH /*|| lcd_print_timeont <= 0.0f*/) {
    ...
    switch (lcdMenuIndex) {
      ...
      case LCDMENU_MAX_ERROR_ANGLE_DEGREES:
        if (incrementButton == HIGH) {
          MAX_ERROR_ANGLE_DEGREES += 0.5f;
        } else if (decrementButton == HIGH) {
          MAX_ERROR_ANGLE_DEGREES -= 0.5f;
        }
        MAX_ERROR_ANGLE_DEGREES = MAX(MAX_ERROR_ANGLE_DEGREES, 0.0f);
        lcd_.setCursor(0, 0);
        lcd_.print("MAX_ER_ANG_FIN");
        lcd_.setCursor(0, 1);
        lcd_.print(MAX_ERROR_ANGLE_DEGREES);
      break;

      case LCDMENU_MAX_ERROR_DISTANCE_FINISH_LINE:
        if (incrementButton == HIGH) {
          MAX_ERROR_DISTANCE_FINISH_LINE += 0.5f;
        } else if (decrementButton == HIGH) {
          MAX_ERROR_DISTANCE_FINISH_LINE -= 0.5f;
        }
        MAX_ERROR_DISTANCE_FINISH_LINE = MAX(MAX_ERROR_DISTANCE_FINISH_LINE, 0.0f);
        lcd_.setCursor(0, 0);
        lcd_.print("MAX_ER_DIST_FIN");
        lcd_.setCursor(0, 1);
        lcd_.print(MAX_ERROR_DISTANCE_FINISH_LINE);
      break;
